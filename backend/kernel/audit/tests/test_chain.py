...
from kernel.audit.diffing import snapshot
...
# --------------------------------------------------------------------------- recording
def test_creating_a_record_captures_its_opening_values(tenant, user):
    with tenant_context(tenant.tenant.id):
...
def test_updating_records_only_the_fields_that_changed(tenant, user):
    with tenant_context(tenant.tenant.id):
...
def test_a_save_that_changed_nothing_records_nothing(tenant, user):
    with tenant_context(tenant.tenant.id):
        before = snapshot(tenant.branch)
        assert record_update(tenant.branch, before, actor=user) is None
        assert AuditEvent.objects.count() == 0


def test_history_for_a_record_reads_newest_first(tenant, user):
    with tenant_context(tenant.tenant.id):
        branch = tenant.branch
        before = snapshot(branch)
    ...
    assert [event.action for event in history] == [AuditAction.PRINT, AuditAction.UPDATE]
