...
## M2.1 Tenancy
- [ ] P0 Models: `Tenant` (id, slug, name, status [provisioning/trial/active/grace/suspended/cancelled/archived], tier [pooled/silo/on-prem], region, default language, timezone `Asia/Kathmandu`, created_by, lead_id), `TenantDomain` (custom domains, verification token, TLS status), `LegalEntity` (PAN, VAT number, registered name en/ne, address, IRD office, VAT registered flag, fiscal-year settings), `Branch` (code, name, address with province/district/municipality/ward, geo, phone, DDA licence ref, opening hours, is_warehouse), `Location` (type: counter/store/rack/shelf/bin/cold room/fridge/freezer/locked cabinet/ward/OT, parent, temperature zone)
- [ ] P0 Nepal address master: 7 provinces, 77 districts, 753 local levels, wards (seeded, bilingual)
- [ ] P0 Tenant context: middleware resolution (domain → subdomain → header), context var, `SET LOCAL app.tenant_id`, RLS policies generated by migration helper for every `TenantScopedModel`
- [ ] P0 DB roles: app role without `BYPASSRLS`; migration role separate; control-plane role with audited bypass
- [ ] P0 Tenant-aware managers, querysets, admin, serializers (reject foreign-tenant FK ids)
...
- [ ] P0 **Tenant isolation test harness**: auto-discovers every tenant-scoped endpoint & asserts cross-tenant access returns 404; runs in CI
- [ ] P0 Tenant provisioning service (idempotent): create tenant → legal entity → HQ branch → default locations → owner user invite → default roles → default settings → numbering series → chart of accounts → tax categories → install plan modules → seed optional demo data → emit `tenant.provisioned`
- [ ] P0 Tenant lifecycle services: suspend (read-only mode middleware), reactivate, cancel, archive, **full data export** (ZIP: JSON + CSV per entity + attachments + PDF registers), purge (after retention, dual approval)
- [ ] P1 Silo tier: DB router selecting connection by tenant; provisioning of dedicated DB; migration runner across silos
- [ ] P1 Custom domain onboarding (CNAME verification, automatic TLS)
...
## M4.7 Sales core
- [ ] P0 Sales document types: Tax Invoice, Abbreviated Invoice (if allowed — CR-IRD-05), Sales Return / Credit Note, Quotation/Estimate, Sales Order, Delivery Note (P1), Proforma (P1)
