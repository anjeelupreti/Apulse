...
- [x] P0 Capture via the service layer: `record_create` / `record_update` produce field diffs, values stay readable (Decimal as text, never float), and secret-looking fields are recorded as changed without revealing either value — **a save that changed nothing records nothing**. **Automatic capture now covers it**: a model is declared tracked once in its app's `ready()`, and every create, change and delete of it is recorded thereafter, without any service having to remember. The entries that matter most are the ones nobody would have thought to add by hand. Deliberately excluded: the stock ledger and the narcotic register, which **are** logs already — append-only, carrying the actor, the document and the reason — so tracking them would write every scan at the counter twice and take the tenant's chain lock on the hot path; and stock balances, which are a derived total. Where a service records the same save in better words ("INV-001 issued, 1,240.00" rather than six fields moving) it wraps the save in `paused()` so the trail says it once
- [~] P0 Explicit business audit events — the full action vocabulary is defined (login, export, print, reprint, override, void, permission change, settings change, impersonation start/end, view-sensitive); **permission changes, invoice issue and cancellation, goods receipt posting and cancellation, credit note issue and drug reclassification are wired up**; the actor now comes from the request context when a caller did not pass one, so an entry raised deep in the stack still names a person; the rest are wired as each flow is built
...
- [ ] P0 Entity "History" tab component reused across all detail pages — *`history_for(instance)` backs it, and with automatic tracking it now has something to show for every master record and document rather than only where a service remembered to log*
...
- [~] P0 Settings service with scopes (platform default → plan default → tenant → legal entity → branch → user) and typed access. A setting is **declared in code** like a permission or a document type — its type, default, both labels and which scopes it may be set at — and reading an undeclared key **raises**, because a typo that silently answers "not set" is how a rule quietly stops being enforced. Values arrive from forms, JSON and spreadsheets as text, so every kind coerces from a string: a stored `"false"` is never read as true. The narrowest scope with a value wins, resolved from **one query per account** and cached by version token like entitlements, so a stale answer is never *read*. `resolve()` says **where** a value came from, because "inherited from the business" and "set for this branch" are different things to the person on the screen. A stored value a later release no longer accepts falls back to the default with a warning rather than taking the counter down. Every change is a `SETTINGS_CHANGE` entry with its reason. **The platform default is the default declared in code** — there is no tenant-less row, because one cannot be protected by row-level security without also letting a single tenant write a value that lands on everybody else; changing a default centrally belongs to the control plane. *Plan defaults arrive with it (Phase 3), as does the settings screen*
...
- [~] P0 Implement CONVENTIONS §API: resource naming, error envelope, idempotency keys (`Idempotency-Key` header stored 24h), optimistic concurrency (`version` field / `If-Match` ETag → `409 CONFLICT`), cursor pagination, `updated_since`, sparse fields, expansions, bulk endpoints. **Idempotency** covers the three states, not two: unseen, **in flight** — a retry arriving while the first attempt is mid-transaction is told to wait, because two concurrent bills is the exact failure being prevented — and finished, which replays with a header saying so. Keys are scoped to account, user and endpoint, so one caller's key can never replay another's answer, and the request body is fingerprinted, so the same key with different content is refused rather than silently answered with the old result. **Failures are not remembered**: a refused sale must stay retryable once the prescription is fixed. **Optimistic concurrency** accepts the version in the body or as `If-Match`, returns the new `ETag`, and refuses a stale edit — last-write-wins loses the other person's change with no trace and no warning. **Shared filters** (`updated_since` as `>=` so a client's own cursor never drops a row written in the same tick; `branch`, always narrowed to what the caller may see whether or not they asked; inclusive `date_from`/`date_to`) — *pending: sparse fields, expansions, bulk endpoints*
...
- [~] P0 OpenAPI annotations complete (examples, error responses, enums with descriptions) — the schema generates with **no warnings and no errors**; document statuses and credit-note kinds are named rather than left as `Status172Enum`, so the generated client's types stay readable — *examples and per-endpoint error responses only on the sales endpoints so far*
...
- [~] P0 Party ledger/statement view (opening, transactions, running balance, ageing) — [REPORT] + share statement via WhatsApp/email. `statement()` returns opening balance, bills as debits, payments and credit notes as credits, and a running balance, ordered by **business date** rather than by when the row was written, because that is the order the customer's own file is in. `ageing()` splits what is owed into current / 1-30 / 31-60 / 61-90 / 90+, which is the shape every collections conversation takes: not "they owe 40,000" but "30,000 of it has been sitting since Ashadh" — *pending: the screen, the PDF and sharing it*
...
## M4.7 Sales core
- [~] P0 Sales document types: **Tax Invoice built, and reachable over HTTP** — draft moves no stock; issuing numbers it, allocates **nearest-expiry batches first**, records **which batch each customer received** so a recall can be answered, and fixes the totals. **Prices come from the price printed on the batch** (falling back to the item for goods that are not batch-tracked), **selling above MRP is refused** because it is an offence, **tax is extracted from the shelf price** rather than added to it, the total rounds to whole rupees with the difference shown, and **reprints are counted so copies can be marked**. A bill that cannot be issued consumes no number, keeping the run gapless. **Credit Note built** — see below. *Pending: Abbreviated Invoice (CR-IRD-05), Quotation, Sales Order, Delivery Note, Proforma*
- [x] P0 **Cancellation must be evidenced by a credit note** for IRD, not only by marking the invoice cancelled (CR-IRD-04). Cancelling now reverses the stock, keeps the number **and** issues a cancellation credit note in its own numbered series, inside the same transaction — marking our own record cancelled is a change to our record, while the note is a document that says what was reversed and why. The cancellation note **moves no stock of its own**, because the reversal already did; doing both would double the stock back in. A bill that has already been partly returned cannot be cancelled at all — credit what is left instead
- [x] P0 Credit sales with limit & overdue checks (block/warn/override by role). **Putting a bill on account is not the money arriving** — it records how the bill was settled at the counter, and the invoice stays fully outstanding, because counting it as received is how a receivables ledger comes to show nothing owed while the shop is owed a fortune. Two separate questions are asked: the **limit** is how much exposure a customer is allowed, and **overdue** is whether they pay at all — a customer well inside their limit who has settled nothing since Baisakh is the worse risk. A **zero limit means no credit**, never unlimited. The decision is *returned* with its figures before it is raised as an error, so the counter can say "you are 300 over and a bill from Shrawan is unpaid" rather than a bare refusal. An override needs **both a reason and a name** and is recorded as an OVERRIDE in the trail: an override nobody signed is the same as no control, and one with no reason is worse, because it looks like a control while teaching everybody the box takes anything. The due date is **frozen onto the bill** when it goes on account, so shortening a customer's terms next year does not turn settled history into a list of late payments
- [~] P0 Sales return: against original invoice (qty ≤ sold − returned), batch returns to stock or to quarantine/damaged, refund method, credit note numbering, reason codes, return without invoice (permission, limits). Credited quantity is checked **against the invoice line**, counting what earlier issued notes already took, so two half-returns cannot add up to more than the whole; an unfinished draft does not block anybody, because a draft is somebody's intention and the customer is waiting. Stock comes back as **the batch it was sold as** — a recall is answered from the batch record, so it has to stay true — and it comes back as a `SALE_RETURN`, not a reversal, because the sale did happen and the goods coming back is a different event. **Quarantine is the default destination**: once a pack has left the premises nobody can say how it was kept, and putting it back on the shelf is a deliberate choice for a sealed pack that never really left. A credit note is priced at **what was charged**, never at today's price, or a refund quietly becomes a discount. Three kinds — cancellation, return and money-only adjustment — and only a return moves stock — **One click credits a whole bill** — draft, fill, number and issue in a single call, refused unless the caller confirms, because it produces a real numbered tax document and moves stock and there is no undo. What it will credit can be read first, so the counter is shown what comes back rather than finding out afterwards. The invoice is never touched: it keeps its number, its lines and its totals, and the correction is a **new entry** — *pending: refund method and the payment side (M4.9), and return without an invoice*
- [~] P0 **Cashier shift / day-end**: open shift with opening cash, cash in/out (petty expenses with category), close shift with denomination count (रू 1000/500/100/50/20/10/5/2/1), expected vs counted variance, per payment mode totals, Z-report print, supervisor sign-off; branch day-close. **One open till per counter, enforced by the database** — a shortfall on a shared drawer belongs to everybody who touched it, which in practice means nobody is ever asked. Money leaving the drawer needs a **witness and a reason**. The count is taken as given and the difference **recorded, never adjusted to match the register**: a till short by four hundred rupees is a fact about the day, and writing it off destroys the only signal anybody had. A difference beyond a rupee will not close without an explanation, and a rupee either way closes quietly, because demanding a paragraph for a rounding artefact only teaches people to type "ok" into the box. The **denomination sheet is kept, not just the total** — "twelve five-hundreds" is checkable against the drawer and "6,000" is not, and a shortfall usually shows as one denomination being wrong. Sign-off is a separate act by a different person. — *pending: the Z-report layout and print, expense categories, and branch day-close across tills*
...
- [~] P0 Property-based tests: sum(lines) = totals, credit note reverses exactly — *a full credit note is asserted to come to exactly what the invoice did, down to the rounding line, across a mixed exempt and standard-rated bill. Generative property tests are still to come*
- [ ] P1 TDS on applicable purchases/expenses

## M4.9 Payments & receipts
- [~] P0 Payment modes master (cash, card, eSewa, Khalti, Fonepay QR, ConnectIPS, bank transfer, cheque, credit, loyalty points, advance/deposit) per branch enabled — eight ship as a starting set (`install_default_modes`) and a tenant adds or disables from there; what the code reasons about is the **kind**, not the name, so only cash is counted in the drawer and only a wallet or card is made to carry its transaction reference — *pending: loyalty points, advances/deposits, ConnectIPS, and per-branch enabling (they are per tenant today)*
- [x] P0 Split payments per invoice; change calculation; tips disabled — half in cash and half on a wallet is one call; the total is checked against **what is still owed**, counting earlier payments and any credit note, so a mistyped second tender cannot leave the accounts holding money nobody claimed. **Change comes only from cash**: handing it back on a card payment is one of the simplest ways to empty a till, so it is refused rather than warned about. Change given is never subtracted from the drawer twice — a hundred handed over for an eighty-rupee bill leaves eighty behind, and eighty is what the payment records
...
- [~] P0 Customer receipts against outstanding invoices (allocation FIFO or manual), advances, refunds. `receive_against()` is deliberately **not** the same call as settling at the counter: settling covers the tenders that make up the bill while the customer is standing there, including putting it on account, which settles nothing and is meant not to; a receipt is the cheque that arrives next month, checked against **what is still owed** rather than what is left to tender — a refund goes **against the credit note**, not the invoice: the note is the document that says money is owed back, and a refund with nothing behind it is indistinguishable from a till being emptied. Payments are **append-only in the database** and a mistake is corrected by recording its opposite — *pending: allocation across several invoices, advances and deposits*
...
- [~] P0 Medicine search: brand, generic, salt, barcode, manufacturer; typo-tolerant trigram; Devanagari & romanized; results show stock, nearest expiry, MRP, schedule badge; < 1 s p95 on 100k items. The counter's search exists and is **separate from the catalogue master list** on purpose, so one can stay fast while the other stays complete. It answers what somebody with a customer waiting actually asks: is there any, what does it cost, when does the nearest one expire. It reports **on hand and available separately** — a box that is there but expired is not the same as no box — and quotes **the price of the batch on the shelf**, which is the figure the bill will charge; a search that quotes the item's price while the bill charges the batch's is a support call waiting to happen. Barcode lookup returns **the pack the barcode was on**, so scanning a box adds a box — *pending: generic and salt search (needs `MedicineProfile` joined in), trigram typo tolerance, romanized input, the schedule badge (core cannot import the pharmacy module — it needs an extension point), and the 100k-item benchmark*
...
- [ ] P0 Rule actions at billing time: `REQUIRE_PRESCRIPTION`, `REQUIRE_PRESCRIBER_REGISTRATION`, `REQUIRE_PATIENT_IDENTITY`, `REQUIRE_REGISTER_ENTRY`, `REQUIRE_PHARMACIST_ROLE`, `MAX_QTY`, `WARN`, `BLOCK` — *`REQUIRE_PRESCRIPTION` and `REQUIRE_PRESCRIBER_REGISTRATION` are enforced at issue; `REQUIRE_REGISTER_ENTRY` drives the narcotic register (M5.5); `REQUIRE_PHARMACIST_ROLE` is enforced for controlled drugs, where an actor without a valid Pharmacy Council registration is refused — an unattributed call is gated at the API layer instead, since refusing it in the service would block the opening-stock import. Patient identity and the quantity ceiling are recorded on the rule and shown to the counter, but not yet acted on*
...
- [x] P0 Register auto-entry on posting sale of KA (and configured KHA/special) items: date, patient name/address/ID, prescriber name & reg no, drug, batch, qty, running balance, dispensing pharmacist (credential-verified), Rx attachment link, signature capture (touch/pen pad optional). **Which समूह is registered comes from the dated `ScheduleRule`, not from a list in the code**, so a DDA notice bringing another group in applies from its own date. Driven off the stock movements rather than the invoice lines, because the movements say which batch actually left the cabinet. Prescriber and pharmacist names are **copied in as text as well as linked**, so a record corrected in 2027 cannot rewrite what a 2026 page says — *signature capture is a device concern for the POS; the attachment becomes a real file reference with M2.7*
- [x] P0 Receipts into register from GRN (supplier, invoice, batch, qty) & adjustments (with witness). The hooks run **inside** the posting transaction: a controlled drug that reached the shelf without its register entry is exactly what the register exists to prevent, so if the entry cannot be written the delivery does not become stock and the sale does not stand
- [x] P0 Running balance per drug/batch per branch; server recomputation & discrepancy alert. The balance row is locked while an entry is appended, so two counters dispensing from one cabinet cannot both read the same balance and write it back. `reconcile_register()` **reports** rather than repairs: a balance that has drifted from the entries means something wrote outside the module, and quietly correcting the number would destroy the only evidence of it — *raising the alert needs the notification service (M2.7)*
- [x] P0 Locked storage location enforced for KA items (location type = locked cabinet) — checked when the stock is received, which is the one moment somebody is looking at where it goes
- [ ] P0 Narcotic stock reconciliation session (physical vs register) with witness, variance reason, approval — *`record_count()` takes the count, requires a witness and a reason, refuses to count a batched drug without naming the batch, and writes only the difference it found; the session document and the approval step are pending*
- [ ] P0 Register views in DDA format; print per drug per period; export PDF/XLSX; one-click inspection export — *`register_page()` returns one drug's lines for a period in the order they were written, which is what a page and an export are both rendered from. **The prescribed format itself is still to be obtained — CR-DDA-02** — and a register that does not match it column for column is not the record the Act asks for*
- [x] P0 Register entries immutable; corrections via annotated correction entry — the database refuses to update, delete or truncate the table, and a correction is a new line pointing at what it corrects with a reason that cannot be left blank. An unexplained correction in a narcotics register is worse than the error it corrects, because it is the shape a diversion takes
